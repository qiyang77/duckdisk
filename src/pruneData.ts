...
export function pruneIrrelevant(
  ...
  //   const reference = origReference ? origReference : node.size;
  //   if (node.children) {
  //     let keep = node.children
  //       .filter((c) => c.size / reference > threshold)
  ...
  //       (c) => c.size / reference <= threshold
  //     );
  //     const smallerItems = consolidate.reduce(
  //       (a, b) => ({ ...a, /*count: a.count! + 1,*/ size: a.size + b.size }),
  //       {
  //         id: uuidv4(),
  //         name: "Smaller Items",
  //         // count: 0,
