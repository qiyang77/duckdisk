...
```bash
MAS_SKIP_SIGNING=1 MAS_BUILD_NUMBER=509 npm run release:macos-store
```

The build is written as a ZIP archive to:

```text
src-tauri/target/mas-store/DuckDisk-0.5.9-MAS-unsigned.zip
```
...
```text
src-tauri/target/mas-store/DuckDisk-0.5.9-MAS.pkg
```
